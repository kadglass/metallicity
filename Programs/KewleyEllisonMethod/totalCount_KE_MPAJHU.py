...
from KewleyEllisonCount import count
import numpy as np
from astropy.table import Table
import sys

sys.path.insert(1, '/Users/kellydouglass/Documents/Drexel/Research/Data/')
...
def totalCount_KE_MPAJHU(fileName, extra_cols, sigma_4363):
	...
	# fileName = 'kias1033_5_KE_MPAJHU_flux.txt'
	# fileName = 'dwarf_voidstatus_MPAJHU_flux_noMean.txt'
	# fileName = 'dwarf_voidstatus_MPAJHU_flux.txt'
	# fileName = 'almostAllGalaxies_KE_MPAJHU.txt'
	# fileName = 'DG_MPAJHU_KE.txt'
	...
	C = {'I06':0, 'PP04':0, 'D02':0, 'pyNeb':0}
	
	# Initialize total table
	tot_names = ['index', 'I06', 'PP04', 'D02', 'pyNeb']
	...
	for i_galaxy in range(N_gal):
		...
		c, flag_3727, flag_OppSp, flag_NH, flag_NeH, StoN, flag_SII, SII_ratio, flag_OIII, OIII_ratio = count(galaxy_data[i_galaxy], sigma_4363)
		
		########################################################################
		#							RECORD RESULTS
		########################################################################
	
		# Update total number of galaxies excluded due to S/N of emission lines
		for line in SN.keys():
		...
		# Add to OppSp flag list
		flag_OppSp_list[i_galaxy] = flag_OppSp
	...
	# Lines needed
	I06_lines = ['OII_3727', 'NeIII_3869', 'OIII_4363', 'H_BETA', 'OIII_4959', 'OIII_5007', 'NII_6548', 'H_ALPHA', 'NII_6584', 'SII_6717', 'SII_6731']
	...
	I06_table['flagOppSp'] = flag_OppSp_list[I06_indices]
	...
	PP04_table['index'] = galaxy_data['index'][PP04_indices]
	
	for column in extra_cols:
		PP04_table[column] = galaxy_data[column][PP04_indices]
	
	for line in PP04_lines:
		for ending in suffixes:
			PP04_table[line + ending] = galaxy_data[line + ending][PP04_indices]
	...
	D02_table['index'] = galaxy_data['index'][D02_indices]
	
	for column in extra_cols:
		D02_table[column] = galaxy_data[column][D02_indices]
	
	for line in D02_lines:
		for ending in suffixes:
			D02_table[line + ending] = galaxy_data[line + ending][D02_indices]
	...
	# print number of galaxies eliminated due to S/N ratio
	print 'S/N < 3 sigma:',SN['S/N invalid']
	...
	print 'S/N of [OIII] 4363 < 1 sigma:',SN['OIII_4363']
	
	# print number of galaxies eliminated due to S/N of H_beta
	print 'S/N of H_beta < 5 sigma:',SN['H_BETA']
